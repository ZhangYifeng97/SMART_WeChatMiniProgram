...
def getFourteenDaysEvents(departmentName):
    ...
    cursor.execute("""
        ...
        WHERE (date(E.Date) == date('now' '+8 hours'))
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+1 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+2 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+3 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+4 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+5 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+6 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+7 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+8 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+9 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+10 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+11 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+12 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
    ...
    cursor.execute("""
        SELECT E.*, COUNT(DISTINCT F.UserID) AS 'Counter'
        FROM %s E LEFT JOIN Favorite F ON E.Date = F.Date AND E.BeginTime = F.BeginTime AND E.Location = F.Location
        WHERE (date(E.Date) == date('now', '+13 day', '+8 hours'))
        GROUP BY E.Date, E.BeginTime, E.Location
        ORDER BY date(E.Date), time(E.BeginTime) ASC
        ;
    """ % departmentName)
